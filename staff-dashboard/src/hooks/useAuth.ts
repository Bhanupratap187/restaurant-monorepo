...
import { User, UserRole } from '@restaurant-monorepo/shared-types';
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      if (response.success && response.data) {
        ...
        // Only allow chef and waiter roles in staff dashboard
        if (!['chef', 'waiter'].includes(userData.role)) {
          throw new Error(
            'Access denied. This dashboard is for kitchen and serving staff only.'
          );
        }
